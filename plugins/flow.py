...
async def handle_file_upload(client, message):
    ...
    if state == "awaiting_convert_file":
        ...
        if is_video:
            ...
            img_buttons = []
            if ext != ".png":
                img_buttons.append(InlineKeyboardButton("Convert to PNG", callback_data="convert_to_png"))
            if ext not in [".jpg", ".jpeg"]:
                img_buttons.append(InlineKeyboardButton("Convert to JPG", callback_data="convert_to_jpg"))
            if ext != ".webp":
                img_buttons.append(InlineKeyboardButton("Convert to WEBP", callback_data="convert_to_webp"))

            # Add them in rows of 2
    ...
    if media:
        ...
        # Note: Telegram's API limit is 2,000,000,000 bytes (1.86 GB), so we use 2000 * 1000 * 1000 to be safe
